...
| MCP | 135 structured tools | remote: `<server>/mcp` (OAuth 2.1, RFC 9728 auto-discovery); local: `npx jjhub@latest mcp serve` (stdio) or `--http <port>` |
...
| WebMCP | 135 tools, progressively loaded | in-browser only, from an open JJHub tab — no separate connection |
...
mcp serve                              MCP server over stdio (135 tools) for agent clients
